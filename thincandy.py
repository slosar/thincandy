...
def rd_cuesta_approx(obh2, ocbh2, onuh2, Nnu):
    ...
    return 55.154 / (
        ocbh2 ** 0.25351
        * (obh2) ** 0.12807
        * np.exp((onuh2 + 0.0006) ** 2.0 / 0.1176 ** 2)
    )
...
class ThinCandy:
    def __init__(self):
        ...
        self.Nr = 50
        self.zar = np.hstack(
            (np.linspace(0, 3, self.Nl), np.linspace(1100, 1200, self.Nr))
        )
        self.aar = 1 / (1 + self.zar)

        # Planck best fit
        # https://www.cosmos.esa.int/documents/387566/387653/Planck_2018_results_L06.pdf
        # Page 14,  Table 1, Combined column
        ## sigma8 = 0.8101±0.0061
        self.Obh2 = 0.02233
        self.Och2 = 0.1198
        self.lnttAs = 3.043
        self.h = 67.32 / 100.0
    ...
    def getModels(self):
        if True:
            self.Copen, self.CopenP = self.GetCosmo({"Omega_k": -0.044})
            print("getting cw")
            self.Cw, self.CwP = self.GetCosmo({"w0": -1.58})
            print("getting cmnu")
            self.Cmnu, self.CmnuP = self.GetCosmo({"m_nu": 0.26})
            # self.Cmnuless = self.GetCosmo ({'m_nu':0.0})
    ...
    def bgplot(self):
        ...
        f, axl = plt.subplots(
            ...
            2,
            facecolor="w",
            gridspec_kw={"width_ratios": (3, 1), "hspace": 0.0, "wspace": 0.05},
            figsize=(10, 6),
        )
            ...
            if "perp" in name:
                for zb, iso, isoe, perp, perpe, par, pare in self.baodata:
                    ...
                    if iso > 0:
                        ...
                        normpar = 299792.45 / (
                            ccl.h_over_h0(self.CBase, 1 / (1 + zb))
                            * self.CBase["H0"]
                            * self.rd(self.CBase)
                        )
    ...
    def apar(self, C):
        ...
        apar0 = (
            1
            / ccl.h_over_h0(self.CBase, self.aar)
            / self.CBase["h"]
            / self.rd(self.CBase)
        )
        return apar / apar0
    ...
    def sn(self, C):
        ...
        ratio = sn / sn0
        fact = (interp1d(self.zar, ratio)(self.snz) / self.snerr ** 2).sum() / (
            1 / self.snerr ** 2
        ).sum()
